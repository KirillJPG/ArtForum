...
export function Card({author,createAt,imageUrl,likes,name,id}:ApiSchemas["ArtResponse"]){
    return (
        <Link to={generateTypedPath(ROUTES["ART"],{artId:id})}>
            <div className="border border-black60 p-2 rounded-md grid gap-1">
                <div className="flex justify-center relative">
                    ...
                    <img src={imageUrl} height={100} width={150} className=""/>
                </div>
                <div className="overflow-ellipsis w-full overflow-hidden "><span className="w-full text-lg capitalize">{name}</span></div>
                <div className="">
                    <Link to={generateTypedPath(ROUTES["USER"],{userId:author.id})} className="hover:brightness-50 duration-300 text-primary z-20 flex gap-1 items-center"><PersonIcon width={22} height={20}/> {author.name}</Link>
                </div>
            </div>  
        </Link> 
    )
